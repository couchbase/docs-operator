...
Changes to a CouchbaseCluster should never be done manually as the Operator will revert the changes if they do not match what is in the configuration. Always update the cluster configuration using the ```kubectl``` command. We recommend using either the ```kubectl replace``` or ```kubectl edit``` command.
If you're using OpenShift, use the ```oc replace``` or ```oc edit``` command instead.
...
It is important to remember that some fields in the Couchbase cluster configuration cannot be edited after the cluster is created. The [Couchbase Cluster Configuration](couchbaseClusterConfig.md) documentation notes such fields in their respective descriptions. The current version does not support field validation.
