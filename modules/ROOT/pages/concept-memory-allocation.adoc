...
For shared nodes, you'll be using xref:reference-couchbasecluster.adoc#spec-servers-pod-resources[Pod resource limits and requests] for each server in the `CouchbaseCluster` configuration.
...
Specifying a value of `2560Mi` or greater for xref:reference-couchbasecluster.adoc#spec-servers-pod-resources[`spec.servers[\].pod.resources.requests.memory`] will ensure that the server pod does not get evicted for using up too much memory.
