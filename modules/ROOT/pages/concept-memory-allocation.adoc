...
In this configuration, xref:couchbase-cluster-config.adoc#spec-cluster-dataservicememoryquota[`spec.cluster.dataServiceMemoryQuota`] is set to `256Mi`, resulting in 256 MB of RAM being reserved for the Data service on each node, for a total of 768 MB across the whole cluster.

As the cluster is horizontally scaled, so is the amount of memory allocated across the cluster.
If you were to scale this cluster by changing xref:couchbase-cluster-config.adoc#spec-servers-size[`spec.servers.size`] to `4`, it would yield 1 GB of memory available to the Data service.
...
The value you specify for xref:couchbase-bucket-config.adoc#spec-memoryquota[`spec.memoryQuota`] will be reserved on each Couchbase Server node that runs the Data service.
...
For shared nodes, you'll be using xref:couchbase-cluster-config.adoc#spec-servers-pod-resources[Pod resource limits and requests] for each server in the `CouchbaseCluster` configuration.
...
Specifying a value of `2560Mi` or greater for xref:couchbase-cluster-config.adoc#spec-servers-pod-resources[`spec.servers[\].pod.resources.requests.memory`] will ensure that the server pod does not get evicted for using up too much memory.
