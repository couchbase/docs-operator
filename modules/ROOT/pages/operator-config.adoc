...
+
This flag is a boolean value that defaults to `true` on Kubernetes, and `false` on OpenShift.
+
.Example:
[source,yaml]
----
...
+
Valid values are `panic`, `fatal`, `error`, `warn`, `info` and `debug`.
The Operator will only log messages of the specified level and higher, i.e. when set to `fatal`, only messages marked `fatal` and `panic` will be logged.
...
By default the Operator constrains Kubernetes rolling upgrades with `PodDisruptionBudget` resources and exec based readiness checks.
This ensures that data is always available (provided buckets have at least one replica).
To facilitate this the Operator places a file on the `Pod` file systems to indicate it is clustered and all data is replicated.
This requires the `pods/exec` RBAC permission.
If your security policy does not allow `pods/exec` and you acknowledge that Kubernetes rolling upgrades may lead to data loss this functionality may be disabled by setting this flag to `false`.
