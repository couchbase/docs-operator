...
This allows orchestration of xref:concept-kubernetes-upgrade.adoc[automatic Kubernetes upgrades].
+
...
events::
The Operator will raise xref:reference-couchbasecluster-events.adoc[events] that are associated with a `CouchbaseCluster` resource, which can be used to synchronize external operations or raise alerts.
+
