...
`backups` is a list of all the repos that exist in the backup archive `{backup-archive}`, which should aid in selecting one to create a `CouchbaseBackupRestore`.
Each entry will require a singular entry for `name` and `full`, where `full` is the first backup taken inside a repo.
