...
`backups` is a list of all the repositories that exist in the backup archive `{backup-archive}`, which should aid in selecting one to create a `CouchbaseBackupRestore`.
Each entry will require a singular entry for `name` and `full`, where `full` is the first backup taken inside a repository.
`incremental` is a list of strings that corresponds to the incremental backups taken after the full backup.
____
...
This field is only applicable to `CouchbaseBackup` objects and tells us the name of the most recent or currently running cron job.
