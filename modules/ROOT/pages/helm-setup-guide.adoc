...
[source,console]
----
helm install mobile --set install.syncGateway=true --syncGatewayExposeServiceType=LoadBalancer couchbase/couchbase-operator
----
...
The example installation commands on this page assume the default namespace is used (they commands don't specify the `-n` option).
