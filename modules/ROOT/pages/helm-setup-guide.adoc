...
The xref:helm-operator-config.adoc[Couchbase Operator Chart] deploys the xref:install-admission-controller.adoc[admission controller] and the Operator itself.
...
. Create an imagePullSecret for retrieving images from openshift registry:
+
[source,console]
----
...
. Create a file named `openshift_values.yaml` with the following values to customize the Oprator Chart:
+
[source,yaml]
----
couchbaseOperator:
...
. Install the Operator Chart using override values from previous step:
+
[source,console]
----
...
. Install the Couchbase Cluster Chart with images from redhat registry:
+
[source,console]
----
helm install \
  --set couchbaseCluster.baseImage=registry.connect.redhat.com/couchbase/server \
  --set  couchbaseCluster.version=5.5.3-3 \
...
. Deploy the Couchbase Server cluster using the traditional method of using a xref:deploying-couchbase.adoc[CouchbaseCluster specification].
+
...
NOTE: For <<deploy-production,production deployments>> where you've manually installed the CRD, you'll need to xref:upgrading-the-operator.adoc#update-crd[manually update] the CRD before you upgrade the Operator via Helm.
