...
This page describes how to set up Helm to properly support the Couchbase Autonomous Operator in your Kubernetes or OpenShift environment.
Setting up Helm consists of installing the Helm client (`helm`) on your computer, and installing the Helm server (Tiller) on your Kubernetes or OpenShift cluster.
...
Two https://docs.helm.sh/using_helm/#three-big-concepts[Helm charts^] are available for deploying Couchbase.
The xref:helm-reference-operator-configuration.adoc[Couchbase Operator Chart] deploys the xref:install-admission-controller.adoc[admission controller] and the Operator itself.
...
. Create a file named `openshift_values.yaml` with the following <<customize-charts,custom override values>> for the Operator Chart:
+
[source,yaml]
----
couchbaseOperator:
...
. Install the Operator Chart with the custom override values:
+
[source,console]
----
...
. Install the Couchbase Cluster Chart with the desired Couchbase images from the Red Hat Container Catalog:
+
[source,console]
----
helm install \
...
. Download the appropriate Operator https://www.couchbase.com/downloads[package^] and unpack it on the same computer where you normally run `kubectl` or `oc`.

. Included in the package is a file called `crd.yaml`.
Use the following command to install it on your Kubernetes cluster:
+
[{tabs}]
====
Kubernetes::
...
Therefore, it's recommended that you keep all of your overrides in a YAML file, and use the the `--values` option instead of the `--set` option when you install the chart.
See <<customize-charts,Customizing Helm Charts>> for more information.
+
[#release-name]
[NOTE]
====
When you install a chart, Helm auto-generates a name for the https://docs.helm.sh/glossary/#release[release^] (usually a unique set of dictionary words).
...
. Deploy the Couchbase Server cluster using the traditional method of using a xref:howto-couchbase-create.adoc[CouchbaseCluster specification].
+
...
The official Couchbase Helm charts are great to use "as-is" for quickly deploying Couchbase on Kubernetes platforms, but you'll undoubtedly need to customize them for your specific development and production needs.
All of the values that are exposed by the charts are available within the `values.yaml` file of the xref:helm-reference-operator-configuration.adoc[Couchbase Operator Chart] and the xref:helm-cluster-config.adoc[Couchbase Cluster Chart].
...
[source,console]
----
helm install --values myvalues.yaml couchbase/couchbase-operator
----
+
...
.Operator Chart
* xref:helm-reference-operator-configuration.adoc[Documentation]
