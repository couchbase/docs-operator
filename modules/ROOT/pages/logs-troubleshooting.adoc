...
To retrieve the logs for the Operator and the entire Kubernetes deployment (nodes, pods, services...), run the following script: https://packages.couchbase.com/kubernetes/0.8.1-beta2/couchbase-kubernetes-support.sh[cb_k8s_support.sh]
...
For OpenShift, refer to the OpenShift installation documentation about https://docs.openshift.org/latest/install_config/cluster_metrics.html[cluster metrics].
...
The easiest way to get `cbcollect` logs is to use the standard logs collection feature in the Couchbase Server Web Console. Go to menu:Logs[Collect Information], select the desired nodes, and click *Collect Logs*.
...
Once the log collection is complete, you can view the log location for each node from the Couchbase Server Web Console by going to menu:Logs[Collect Information] and clicking *Show Current Collection*. You can then run a command like the following for each node in the cluster to collect their logs.
