...
Couchbase metrics may be collected by the xref:howto-prometheus.adoc[Couchbase Prometheus Exporter] or the native Prometheus Endpoint that is exposed in Couchbase Server versions 7.0+.

When using the Prometheus Exporter, the `CouchbaseCluster` resource must have xref:resource/couchbasecluster.adoc#couchbaseclusters-spec-monitoring-prometheus-enabled[`couchbaseclusters.spec.monitoring.prometheus.enabled`] set to `true` in order for Couchbase metrics to be collected by the custom metrics API service.
...
Couchbase Server version 7.0+ expose a Prometheus endpoint which can be used in conjunction with https://prometheus-operator.dev[Prometheus Operator^].

To configure your cluster for use with the Prometheus Operator, you must first install the https://prometheus-operator.dev/docs/prologue/quick-start/#get-the-kube-prometheus-project[`kube-prometheus` Project^].
...
<.> Name of the Couchbase console service which exposes cluster metrics.
Requires xref:resource/couchbasecluster.adoc#couchbaseclusters-spec-networking-exposeadminconsole[`couchbaseclusters.spec.networking.exposeAdminConsole`] is enabled to allow the creation of a service pointing to the Couchbase admin port.
<.> Provides authentication to Prometheus for metric collection.
This secret must be created in the same namespace as the Prometheus Operator, that defaults to `monitoring`.
<.> Namespace in which the Couchbase console service is running.
<.> Match labels of the Couchbase console service.
