...
When data topology is restored to the cluster, two things happen, resources representing the data topology are atomically applied to the cluster, and management of those resources is enabled.
...
[source,console]
----
$ cao save --cluster my-cluster --filename my-cluster-save.yaml
----

The `cao` tool will connect to the named cluster defined by the `--cluster` flag.
...
It is also possible to do a _sub-tree save_, that limits the scope to all resources contained within a single bucket, or scope.
For example, a sub-tree save of a bucket will contain all scopes and collections within it, and a sub-tree save of a scope will contain only the collections within that scope.

[IMPORTANT]
====
Due to the design of scopes and collections for Kubernetes, a default collection can only be preserved by also including its parent scope.
When taking a sub-tree save of a scope, the default collection will be discarded.
====

For more information on the `cao save` command, and all supported flags, read the xref:tools/cao.adoc#cao-save-flags[reference documentation].

==== Restoring a Save File

Restoring a save file to a cluster is equally easy as saving:

[source,console,subs="verbatim,macros"]
----
$ cao restore --cluster my-cluster --filename my-cluster-save.yaml
...
By default, this tool will never delete any resource, this is a safe mode of operation that ensures no data will be accidentally deleted.
You can change the merge strategy to delete resources that appear in the cluster but not in the save file.
This mode of operation is for enforcement of schema, and ensures that unneeded data will be cleaned up.

For more information on the `cao restore` command, and all supported flags, read the xref:tools/cao.adoc#cao-restore-flags[reference documentation].
...
The use of label selectors is two-fold.
...
Second, this allows atomic updates of the entire topology, and rollback in the event of an error.
====
...
You should be familiar with the concepts of xref:concept-scopes-and-collections.adoc[buckets, scopes and collections].
...
This is an inefficient model for our purposes, so instead we introduce a root node, then our forest becomes a single tree.

.Restore Data Model
image:save-restore-data-model.png[]

When we first start processing the save file, two things happen.

First, the save data is processed and it is organized into a tree, the _requested tree_.
This may take the form of a full-tree -- where buckets are children of the root node -- or a sub-tree -- where scopes or collections are children of the root node.
...
When merging trees together in the next phase, it's far easier to consider full-trees, where at each level of the tree, the resource types are the same.
When provided with a full-tree, this can just be left as-is.

Sub-trees however need to be "attached" to some point in the hierarchy, yielding an _effective requested tree_.

.Example Splice
image:save-restore-splicing.png[]

In the above example, a sub-tree save of scopes and collections is spliced into the existing tree to yield the _effective requested tree_.
...
Merging is the most important step in the process.
It takes the current tree and merges it with the effective requested tree to yield a merged tree.
The end product is a single tree that contains all nodes that exist in both input trees, but with an action associated with it.

The default operation is to merge trees so that nothing is deleted:

.Merging with the Merge Strategy
image:save-restore-merge-merge.png[]
...
image:save-restore-merge-replace.png[]
...
Once the merge tree is generated, and the user has given the go ahead, it is processed into a _compacted tree_.
...
Likewise, scopes can be combined into scope groups if they contain the same set of child collections.
...
The final step is to commit the data topology and make it live.
...
The Couchbase cluster is then updated to manage buckets, and select those that we have just created (with a label selector).
...
[IMPORTANT]
====
Because multiple Couchbase clusters can run in the same namespace, there is a danger that any resource created, or deleted, by a restore may be erroneously picked up by another cluster.
It is your responsibility to ensure any other clusters in this namespace have a unique bucket/scope/collection label selectors that will not be affected by this synchronization operation.
====
...
[source,console]
----
$ kubectl patch couchbasecluster my-cluster --type=merge -p '{"spec":{"buckets":{"managed":false}}}'
----
...
$ kubectl patch couchbasecluster my-cluster --type merge -p '{"spec":{"buckets":{"synchronize":true}}}'
----
...
[source,console]
----
$ kubectl patch couchbasecluster my-cluster --type=merge -p '{"spec":{"buckets":{"synchronize":false,"managed":true}}}'
----
