...
This tutorial assumes that the installed resources are present, and also leverages the xref:reference-cbopcfg.adoc[`cbopcfg`] command line tool that comes with the Autonomous Operator binary package.
+
...
The name used in this example is the same as the default one created by xref:reference-cbopcfg.adoc[`cbopcfg`] during standard xref:install-kubernetes.adoc[installation], so we'll just reuse it to make integration simpler.
...
The xref:reference-cbopcfg.adoc[`cbopcfg`] tool usually handles this for you, however, as TLS generation is delegated to a 3rd party, we need to be aware of it.
Again, like the secret name, this is the default service name created by xref:reference-cbopcfg.adoc[`cbopcfg`], so we'll just keep this as is.
...
When you install the Autonomous Operator, the xref:reference-cbopcfg.adoc[`cbopcfg`] tool automatically creates a default TLS configuration for you.
...
Next, locate the `MutatingWebhookConfiguration` and `ValidatingWebhookConfiguration` resources.
These need to be updated to include the signing CA's certificate.
