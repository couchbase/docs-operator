...
As mentioned in the <<before-we-begin, prerequisites>> section, this tutorial assumes that you've already performed a basic xref:install-kubernetes.adoc[installation] of the Kubernetes Operator.
...
<.> The `serverSecretName` tells the Kubernetes Operator to use the cert-manager issued secret defined in the previous section <<create-the-Couchbase-server-certificate>>.
