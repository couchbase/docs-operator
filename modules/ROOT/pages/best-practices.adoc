...
This covers the Couchbase Server logs only so pods that fail cannot be recovered, however the logs can still be accessed and downloaded with the xref:reference-cbopinfo.adoc[support tool].
...
[IMPORTANT]
====
As all storage is ephemeral, this means that Couchbase Server logs are highly likely to be unavailable in the event of a crash.
This may make support of such deployments particularly difficult, so use this deployment method with caution.

Streaming of Couchbase Server logs to an aggregator is planned for a later release, and will allow server logs to be collated, and provided as a support request.
====
...
When you deploy a cluster, you do so in a https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/[namespace^]. The Operator needs to be given a fairly broad set of permissions in order to dynamically create pods, services, etc. It is recommended that the Operator, and the clusters that it manages, be segregated in their own name space. This limits the set of resources that the Operator can affect, and that xref:reference-cbopinfo.adoc[support tools] can collect data about, which aids in security compliance situations where confidential information may be leaked via the Kubernetes API operations.
