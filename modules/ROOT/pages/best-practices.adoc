...
For more information, refer to xref:server-groups.adoc[About Using Couchbase Server Groups With the Operator].
...
Production Stateless::
These deployments require the `logs` volume mount to be defined.  This covers the Couchbase Server logs only so pods that fail cannot be recovered, however the logs can still be accessed and downloaded with the xref:cbopinfo.adoc[support tool].  The `query`, `search` and `eventing` services may be deployed with this method, if they are not running with a stateful service.  Logs are only retained in the event of a pod failure.  They will be automatically deleted due to a user initiated action e.g. scaling the cluster down.
...
IMPORTANT: Production clusters may be deployed as combination of _Production Stateful_ or _Production Stateless_ deployments depending on the services running on the individual server class configurations.  The `default` and `logs` mounts are mutually exclusive.  If any server class is detected by the cluster validation to be supportable, then it will also enforced that all other server classes are supportable too.
...
NOTE: There will be a slight reduction in performance and latency when using a network-based storage backend.
...
vm.swappiness::
For systems with a swap file, this controls how aggressively the kernel will transfer unused application memory pages from physical RAM onto disk. It is highly recommended that this value be set to `0` to improve the data service performance, and by extension dependent services.
