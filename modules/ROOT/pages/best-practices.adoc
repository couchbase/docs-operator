...
In this mode of operation, the Operator can recover a partially down cluster -- where Couchbase server is unable to automatically failover the pods -- by forcing the down pods out of the cluster with the `couchbaseclusters.spec.recoveryPolicy` attribute.
