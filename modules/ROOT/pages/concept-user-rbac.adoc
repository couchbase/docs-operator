...
Users are created when a xref:reference-couchbaseuser.adoc[`CouchbaseUser` resource] is bound to one or more xref:reference-couchbasegroup.adoc[`CouchbaseGroup` resources] using a xref:reference-couchbaserolebinding.adoc[`CouchbaseRoleBinding` resource].
...
The name of a bucket can be added to a bucket role to explicitly define which bucket the role should be applied to.
...
The Autonomous Operator can be used to enable this functionality when an `ldap_group_ref` is specified within the `CouchbaseGroup` resource.
...
metadata:
  name: data_role
spec:
  ldap_group_ref: cn=mygroup,ou=Groups,dc=example,dc=com
  roles:
  - name: cluster_admin
----
...
Note that you'll need to have LDAP connectivity configured in the xref:reference-couchbasecluster.adoc[`CouchbaseCluster` resource] in order use this functionality.
