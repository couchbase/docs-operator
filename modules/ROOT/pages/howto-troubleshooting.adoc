= View Operator Logs
:page-aliases: logs-troubleshooting
...
* xref:reference-couchbasecluster-status.adoc[Cluster Status and Conditions]
* xref:reference-couchbasecluster-events.adoc[Understanding Conditions and Events]
* xref:reference-operator-configuration.adoc[Operator Configuration]

== Full Deployment Logs

The Operator is distributed with a support tool which can automatically collect resources, logs and events from the Kubernetes cluster for use in support cases.  It is also capable of collecting logs from Couchbase server instances via `cbcollect_info`. Please see the xref:reference-cbopinfo.adoc[documentation].
...
You can even use the deployment to show the logs.
Since there is only one instance of the Operator in the deployment, the underlying command will automatically select the correct pod:
...
The Couchbase operator serves profiling data on it's default listen address `localhost:8080`.  You can access this endpoint by running a remote shell or forwarding the port to your local system.
...
+
--
Access Go routine stack traces via a shell:
[source,console]
----
