...
This field defines wether auto-compaction should be performed in parallel.
...
Tomestones are retained so that eventually consistent consumers can observe document deletion in the event of a disconnection.
...
The CA certificate must be the root authority for a server certifcate defined in `spec.networking.tls.static.member.serverSecret`.
The xref:tls.adoc[TLS documentation] contains detailed information on generating valid certificates for your cluster.
...
If set to `enable` then Couchbase Server will request a client certifcate and the clients may return one in order to authenticate.
...
____
_Field rules:_ If using client certifcates this list should contain at least one item.
____

=== spec.networking.tls.clientCertificatePaths[].path

This field defines where to extract the username from.
Couchbase Server currently allows usernames to be defined in the certifcate subject common name (CN), or subject alternative names (SAN).
...
This field defines whether the username is delimted.
...
If specified the autentication secret must refer to a Kuberenetes secret containing the keys `username` and `password`.
...
If replications to the remote cluster are to be encrypted then `spec.xdcr.remoteClusters[].hostname` must specify the TLS port (18091 by default), and at least the CA certificate.
