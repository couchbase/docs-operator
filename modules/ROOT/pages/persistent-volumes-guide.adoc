...
This decoupling provides a higher degree of resilience for data within the Couchbase Cluster in the event that a node goes down or its associated Pod is terminated.  Refer to xref:node-recovery.adoc[Node Recovery] additional information about the recovery of Pods with Persistent Volumes.

== Setup Persistent Volumes

*Prerequisite:* Kubernetes must be set up to support clusters with persistent volumes, refer to the xref:persistent-volume-setup.adoc[Persistent Volume Setup Guide]
      ...
      spec:
        ...
        resources:
          requests:
            storage: 1Gi
----
This spec uses a StorageClass named `gp2` to request persistent volumes of size 1Gi for both the `default` and `data` mounts specified in `volumeMounts`. Claims can request volumes from various types of storage systems as identified by the storage class name.  It is also possibl to claim manually created volumes as outlined in the xref:persistent-volume-setup.adoc[Persistent Volume Setup Guide].
...
* For deployments in clouds with multiple availability zones see xref:persistent-volumes-zoning-guide.adoc[Persistent Volume Zoning Guide].
