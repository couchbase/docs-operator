...
For 8.5.0+ clusters, file based rebalances are enabled by default. The Data Service may copy vBucket files directly from the source node to the destination node during the backfill phase of a vBucket move, instead of streaming the data over DCP. File based moves are significantly faster and use less CPU, memory and network.
Use this annotation if you want to opt out of this behaviour and keep using DCP. 
