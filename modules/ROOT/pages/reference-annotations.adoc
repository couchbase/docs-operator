...
==== `cao.couchbase.com/autoCompaction.magmaFragmentationPercentage`
Used to set the auto-compaction magma fragmentation percentage, which determines the point when database compaction is triggered for buckets with a magma storage backend. This annotation can be applied to clusters and buckets and takes a number value between 10 and 100. If applied to a cluster, it must be running Couchbase Server 7.1.0 or later or the annotation will be rejected. If set on a bucket, it will override the cluster level auto-compaction settings when the bucket is using the magma storage engine, or it will be ignored. If this annotation is not set, magma fragmentation percentage will default to 50.
