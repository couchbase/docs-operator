...
NOTE: The following guide assumes you have administrative privileges for the Kubernetes cluster you are deploying into for testing purposes.  Please refer to the xref:kubernetes-rbac.adoc[RBAC doumentation] and make the necessary changes if your development environment differs.

== Creating a ClusterRole

The first step for installing the Couchbase Operator is to create a _ClusterRole_ that allows the Operator to access the resources it needs to run. Since the Couchbase Operator might run in many different namespaces, it is best to create a ClusterRole because you can assign that role to a _ServiceAccount_ in any namespace.
...
After the ClusterRole is created, you need to create a ServiceAccount in the namespace where you are installing the Couchbase Operator, and then assign the ClusterRole to that ServiceAccount using a _ClusterRoleBinding_. In this guide we will use the `default` namespace to create the ServiceAccount.
...
NOTE: If you would prefer to use a _RoleBinding_ instead of a ClusterRoleBinding, then the CRD needs to be installed separately from the Couchbase Operator. This can be accomplished by removing the `-create-crd argument` from the `operator.yaml` file and uploading the CouchbaseCluster CRD separately.
