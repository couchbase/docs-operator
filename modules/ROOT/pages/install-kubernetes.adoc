...
This guide walks through the recommended procedure for installing the Couchbase Autonomous Operator on a Kubernetes cluster that has RBAC enabled.
...
If you have a Kubernetes environment that already has a custom setup (non-default), you should be able to modify a few of the parameters in the various commands and configuration files mentioned in this guide in order to fit your requirements.
...
The first step for installing the Operator is to create a https://kubernetes.io/docs/reference/access-authn-authz/rbac/#api-overview[cluster role] that allows the Operator to access the resources it needs to run. Since the Operator might run in many different https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/[namespaces], it is best to create a cluster role because you can assign that role to a https://kubernetes.io/docs/reference/access-authn-authz/rbac/#service-account-permissions[service account] in any namespace.

To create the cluster role for the Operator, run the following command:

[source,console]
----
kubectl create -f https://packages.couchbase.com/kubernetes/0.9.0-beta3/k8s-rbac/cluster-role.yaml
----
...
[source,console]
----
kubectl create -f https://packages.couchbase.com/kubernetes/0.9.0-beta3/k8s-rbac/operator.yaml
----
