...
[source,console]
----
kubectl create -f operator-cluster-role.yaml
----
...
NOTE: If you would prefer to use a _role binding_ instead of a cluster role binding, then the CRD needs to be installed separately from the Operator. This can be accomplished by removing the `-create-crd` argument from the `operator-deployment.yaml` file and uploading the CRD separately.
