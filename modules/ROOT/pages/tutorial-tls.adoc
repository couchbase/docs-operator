...
The key/certificate pair can be found in `pki/private/couchbase-server.key` and `pki/issued/couchbase-server.crt` and used as `pkey.pem` and `chain.pem`, respectively, in the `couchbaseclusters.spec.networking.tls.static.serverSecret` parameter.
