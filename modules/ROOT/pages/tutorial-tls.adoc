...
Please consult the xref:server:learn:security/certificates.adoc[Couchbase Server documentation^] for additional details on certificate and TLS configuration.
...
The use of passphrase protected keys require Server 7.1.
Passphrase protected keys may be generated with `openssl` as PKCS#8 RSA formatted.
A `ca.crt` and `ca.key` is required for the following steps to sign the server certificates.
Refer to <<easyrsa>> section to initialize and create the CA certificate/key.
...
* xref:howto-tls-passphrase.adoc[How to configure Password Protected TLS]
