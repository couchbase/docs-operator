...
In particular, this page describes the contents of the chart's https://github.com/couchbase-partners/helm-charts/blob/{helm-branch}/charts/couchbase-operator/values.yaml[values.yaml^], which contains the chart's default values.
...
----
helm install my-release -f values-persistent.yaml {helm-repo}
----
...
This value determines whether xref:concept-tls.adoc[legacy or Native TLS] format will be used.

_Value rules:_ The `tls.legacy` is a boolean which defaults to `false`.
...
[source,console, subs="attributes,verbatim"]
----
helm install my-release --set tls.generate=true -f values-persistent.yaml {helm-repo}
----
...
Refer to xref:concept-tls.adoc[TLS Concept] documentation for additional information about available security options.

[source,console, subs="attributes,verbatim"]
----
helm install  my-release -f tls_values.yaml {helm-repo}
----
...
=== `autobind`
_Value rules:_ The `user.<name>.autobind` value is a boolean.
...
[source,console, subs="attributes,verbatim"]
----
helm install --set admissionController.imagePullSecrets={pullsecret1,pullsecret2} {helm-repo}
----
|===
