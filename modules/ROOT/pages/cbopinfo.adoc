...
--operator-image::
If specified this sets the Operator image name.  It is used to match containers running the Operator for collection of runtime information.  If not set it will default to `couchbase/operator:1.0.0`.
...
--server-image::
If specified this defines the Couchbase Server image to use when collecting logs from detached log volumes.  If not set, it will default to `couchbase/server:enterprise-5.5.1`
...
$ cbopinfo --namespace myproject --operator-image couchbase/operator:1.0.0-1
