...
--server-image <image>::
If specified this defines the Couchbase Server image to use when collecting logs from detached log volumes.  If not set, it will default to `couchbase/server:enterprise-6.0.1`
...
$ cbopinfo --namespace myproject --operator-image couchbase/operator:1.2.0-2
