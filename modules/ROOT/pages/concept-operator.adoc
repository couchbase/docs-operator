...
While requiring less operational overhead, and upgrade of the Operator will affect all Couchbase clusters running on the platform, including other Couchbase clusters managed by Operators that are namespace scoped. If this is the case then both Operators will attempt to manage the cluster resulting in the cluster being in a bad state. So use this deployment method with caution.
...
* TLS Secrets
* Logging Configuration Secret (xref:resource/couchbasecluster.adoc#couchbaseclusters-spec-logging-server[`couchbaseclusters.spec.logging.server`])
