...
==== Hibernation & Pausing
Pausing and hibernating the cluster takes priority over everything else. If `spec.paused` is `true` then the Operator will pause the cluster without taking any further action and takes precedance over hibernating. If `spec.hibernate` is `true` then the Operator will hibernate the cluster without taking any further action.
