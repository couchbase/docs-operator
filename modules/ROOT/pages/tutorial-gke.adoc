...
[IMPORTANT]
====
The Kubernetes Version should match a supported *stable* version.
Run the below command to find the supported *stable* versions:
...
$ gcloud container clusters create my-cluster-us-east1 \
  --cluster-version <stable-kubernetes-version> \
  --region us-east1 \
  --network my-network \
