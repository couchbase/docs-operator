...
The Operator automatically resolves most issues.
However, enabling Manual Intervention Required (MIR) places the Couchbase cluster into a special state when the Operator cannot reconcile issues outside its control.
In this state, the cluster requires manual intervention by a user.
The MIR Watchdog runs the additional checks that detect and trigger this state.
...
For this reason, the default value is set to `false`.
