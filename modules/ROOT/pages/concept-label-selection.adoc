= About Couchbase Custom Resource Label Selection
...
.Default selection for a single cluster when no resource labels are defined
...
.Default selection for multiple clusters when no resource labels are defined
image::selection-default-shared.png[]
...
.Label selection with multiple clusters
image::selection-label-selection-basic.png[]

NOTE: You might notice that in the above configuration examples, the cluster has the same name as the bucket label (`my-cluster`).
...
. Multiple `CouchbaseCluster` resources can have the same label selector defined.
When a resource has the shared label, all of the `CouchbaseCluster` resources will select that resource.

. A resource can have multiple different labels.
When different `CouchbaseCluster` resources use different label selectors, they will share a resource if it has the labels required by both clusters.
