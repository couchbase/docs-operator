...
Kubernetes users often need to check the status of various objects (such as pods, deployments, and StatefulSets) that have been deployed inside their Kubernetes clusters. Status checking is done by _describing_ one of the objects in the cluster using `kubectl describe` command.
...
Status:
  ...
  Current Version:		enterprise-5.5.1
  Members:
    Ready:
      cb-example-0000
      cb-example-0001
...
See xref:admin-console-access.adoc[Accessing the Couchbase Web Console] for information on how to expose and access the Couchbase Server Web Console.
...
A list of buckets currently active within the Couchbase cluster. The values of the bucket status are the same as the values provided in the bucket specification.  See xref:couchbase-cluster-config.adoc[CouchbaseCluster Configuration] for details about the bucket specification. When buckets from the specification are added, updated, or removed the following events are generated, respectively:
...
See xref:conditions-and-events.adoc[Conditions and Events] for more information about these conditions and their statuses.
...
The size of the Couchbase cluster. When you change this value in the cluster specification, this value is also updated as members are added to the Couchbase cluster.  If an error occurs while the Operator is attempting to conform the cluster to the desired size, check the `Conditions` section for information about the current state of the cluster.  See xref:admin-console-access.adoc[Accessing the Couchbase Web Console] for information on how to manually access the cluster and collect logs when additional troubleshooting is needed.
...
A list of conditions reflecting the current state of the Couchbase cluster. Each Condition item is denoted by a `Type` along with an associated `Status`. The various conditions and their statuses are documented in xref:conditions-and-events.adoc[Conditions and Events].
