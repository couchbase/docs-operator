...
If specified this sets the Operator image name.  It is used to match containers running the Operator for collection of run time information.  If not set it will default to `couchbase/operator:{operator-version}`.

--operator-rest-port <port>::
If specified this sets the Operator REST API port.  It is used for collection of run time information.  If not set it will default to _8080_.
...
$ cbopinfo --namespace myproject --operator-image registry.connect.redhat.com/couchbase/operator:{operator-version}
