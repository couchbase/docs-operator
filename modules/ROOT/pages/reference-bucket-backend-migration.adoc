...
== Migrating Couchbase Bucket storage Backends
...
=== Using managed CouchbaseBuckets
...
Note that the Magma backend has additional requirements on the bucket:
* Bucket memory quota being 1024MB or more
* Bucket must use the "fullEviction" eviction policy.
...
=== Using Managed CouchbaseBuckets with defaultStorageBackend cluster annotation
...
=== Migrating unmanaged buckets with targetUnmanagedBucketStorageBackend cluster annotation
The Couchbase Operator can also migrate unmanaged buckets. However this will target all bucket in the cluster. It requires the cluster to have bucket management disabled. It also requires that the buckets meet the neccesary requirements to be migrated to magma:
* Bucket memory quota must be 1024MB or more
* Bucket must use the "fullEviction" eviction policy.
...
=== Migrating from Magma to Couchstore
When migrating from Magma to Couchstore, you must first set the `cao.couchbase.com/historyRetention.collectionHistoryDefault` annotation to `false` on the bucket.
This must be done before the migration is started.

Therefore, the migration must be done in two steps. First, apply the annotation to the yaml and apply the yaml to the cluster.
[source,yaml]
----
apiVersion: couchbase.com/v2
kind: CouchbaseBucket
...
Then, change the storage backend to couchstore and apply the yaml again.
...
If this is not done, the admission controller will reject the yaml change with the error below. If this occurs, change
the storage backend back to magma and then follow the steps above.

`spec.storageBackend backend can only be changed from magma to couchstore if history retention is first disabled on the bucket: bucket doesn't have cao.couchbase.com/historyRetention.collectionHistoryDefault annotation set to false`