...
In 2.8.1, we added `cao.couchbase.com/autoCompaction.magmaFragmentationPercentage`.
This ticket requires the value to be added to the CRD’s, and marked as deprecated (Or remove from the documentation) as an annotation.
...
You can now specify to do an EvictionPolicy change while the bucket is online via swap rebalance.
...
[KnownIssue] The CouchbaseCluster CRD has grown to a size that does not allow client side apply to be used.
