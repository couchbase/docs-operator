...
This is because the Operator will be able to recreate them from the cluster `ConfigMap`, metadata attached to the persistent volume claims, and the `CouchbaseCluster` resource itself.
...
<.> The backup has registered and has take a snapshot of our persistent volumes.
...
This is because we backed the `ConfigMap` that is used to determine leadership.
