...
https://velero.io/[Velero^] is a full system backup and restore tool.
Unlike a Couchbase backup -- that is only concerned with data and requires duplication -- https://velero.io/[Velero^] allows an almost instantaneous backup of your data and Kubernetes resources.
...
[IMPORTANT]
====
Use of https://velero.io/[Velero^] backup will only work with Operator version 2.0.0 or greater.
====

== Installing Velero

Velero is a stand-alone binary that can be downloaded from their https://velero.io/[website^].
This tutorial was tested using https://velero.io/[Velero^] version 1.2.0, using the Google Kubernetes Engine (GKE).
...
[source,console]
----
$ cbopcfg --namespace default -no-operator | kubectl create -n default -f -
...
[source,console]
----
$ ./build/bin/cbopcfg --namespace test -no-admission | kubectl create -n test -f -
...
The Operator will begin to provision the cluster.
In the mean time let's have a look at the configuration:
...
<.> Buckets are managed, and we have created a `default` one for use.
<.> The cluster has 3 nodes, totally arbitrary, but is a recommended starting point.
<.> Persistent volume mounts are in use, therefore data is persisted on a volume.
    https://velero.io/[Velero^] is able to take snapshots of the backing storage.

Once the cluster has provisioned you can populate it with some data to prove that backups have worked.
List the pod names and forward the console port:
...
From here connect to `\http://localhost:8091` and log into the console.
Try manually adding some documents to the `default` bucket.
...
$ velero backup create \
  ...
  --include-resources couchbaseclusters.couchbase.com,couchbasebuckets.couchbase.com,persistentvolumes,persistentvolumeclaims,secrets,deployments,roles.rbac.authorization.k8s.io,rolebindings.rbac.authorization.k8s.io,serviceaccounts,configmaps # <.>
----

<.> `test` is the name of the backup we wish to create.
    This is globally scoped so needs to be unique.
<.> We specify that this backup should only look at the `test` namespace.
...
<.> We specify exactly what resources to backup.
    ...
    Backing up persistent volume claims and config maps will save all the persistent metadata required by the Operator for recovery.
...
This is because the Operator will be able to recreate them from the cluster config map, metadata attached to the persistent volume claims, and the `CouchbaseCluster` resource itself.
...
<.> The backup has registered and snapshotted our persistent volumes.

== Disaster Strikes!

The Operator can easily handle pods and persistent volumes being deleted.
What it can't cater for is someone deleting everything, which is where https://velero.io/[Velero^] comes in.
So let's simulate total devastation to see https://velero.io/[Velero^] in action:
...
This is where all the hard work correctly configuring the backup -- and testing it -- pays dividends.
In a real-world disaster recovery situation you are probably panicking, and thinking irrationally.
https://velero.io/[Velero^] makes restoration very easy, and therefore virtually fool-proof while under pressure:
...
After a few short moments you will see the namespace and resources being recreated.
Eventually the Operator will restart as its deployment recognizes it has not pods running:
...
This is because we backed the config map that is used to determine leadership.
...
They should be present and `Bound`, ready to be used.
...
After a few moments the Operator will recreate a Couchbase pod, then restore the rest of the cluster.
You can now reconnect to the Couchbase web console and check that the documents you inserted earlier are fully restored.

== Further Reading

* https://velero.io/[Velero homepage^]
