...
____
_Field rules:_ The `exposeAdminConsole` field is optional and defaults to `false` if not specified.  If set to `true` the specification must also have the <<spec-networking-adminconsoleservices>> property defined.
____
...
Refer to the xref:tls.adoc[TLS documentation] for more information.
...
The xref:tls.adoc[TLS documentation] contains detailed information on generating valid certificates for your cluster.
