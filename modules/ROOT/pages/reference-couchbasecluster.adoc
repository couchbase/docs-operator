...
:page-aliases: couchbase-cluster-config
...
This parameter may be set to `true` if you decide to make manual changes to the cluster.
...
The default value if not specified, `RollingUpgrade`, upgrades one pod at a time.
...
The value `ImmediateUpgrade` can be used to upgrade all pods at the same time.
Immediate upgrades are much faster than rolling upgrades.

____
_Field rules:_ The `upgradeStrategy` field is optional and must be one of `RollingUpgrade` or `ImmediateUpgrade`.
____
...
When set to true, as defined by <<spec-hibernationstrategy, `spec.hibernationStrategy`>>, the cluster is deprovisioned.

For more information, read the xref:concept-hibernation.adoc[hibernation concept page].
...
____
_Field rules:_ This field is optional and must be `Immediate`
____
...
When `PrioritizeDataIntegrity`, the default, is set the Operator will take a cautious approach to recovery.
...
When `PrioritizeUptime` is set, the Operator will wait for a period beyond the requested auto-failover timeout before taking action.
...
This policy may cause data loss, however is useful for recovery of ephemeral clusters -- where no persistent volumes exist and data is lost anyway -- and xref:concept-hibernation.adoc[hibernation] where Couchbase server refuses to failover ephemeral pods.

____
_Field rules:_ This field is optional and must be one of `PrioritizeDataIntegrity` or `PrioritizeUptime`.
____
...
This field is a Kubernetes https://v1-17.docs.kubernetes.io/docs/reference/generated/kubernetes-api/v1.17/#podsecuritycontext-v1-core[`PodSecurityContext`^] object which is attached to all pods that are created by the Operator. If unspecified, this will default to the `couchbase` user, mount attached volumes as that user, and ensure that the containers are running as non-root. You may override the default behavior if using a custom container image or for testing purposes.
...
A server configuration is considered stateful if it contains one of `data`, `index`, `search`, `eventing`, or `analytics`.
A bucket is considered stateful if it is one of `CouchbaseBucket`, or `CouchbaseMemcachedBucket`.
Server configurations with stateful services or stateful buckets are only permitted to autoscale when `enablePreviewScaling` is `true`.


A server configuration is considered stateless if it contains only the `query` service.
A bucket is considered stateless if is a `CouchbaseEphemeralBucket`.
When `enablePreviewScaling` is `false` only stateless server configurations associated with stateless buckets are permitted to autoscale.
...
____
_Field rules:_ The `enablePreviewScaling` field is an optional boolean.  The default value is `false`.
____
...
This functionality is now provided by the <<spec-cluster-indexer-storagemode>> property.
====
...
____
_Field rules:_ This field is optional and must be an integer greater than or equal to `0`.
____
...
____
_Field rules:_ This field is optional and must be one of `silent`, `fatal`, `error`, `warn`, `info`, `verbose`, `timing`, `debug` or `trace`.
____
...
This defaults to `2`.
____
_Field rules:_ This field is optional and must be an integer greater than or equal to `1`.
____
...
____
_Field rules:_ This field is optional and must be a valid https://golang.org/pkg/time/#ParseDuration[duration^] greater than or equal to `1ms`.
____
...
The storage mode can only be modified when no server classes exist that contain the `index` service.

____
_Field rules:_ This field is optional and must be one of `memory_optimized` or `plasma`.
____
...
When the <<spec-networking-exposeadminconsole>> property is set, the Operator will generate a service that exposes the administrator console.
This field allows the definition of a base https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.19/#service-v1-core[`Service` resource^].
...
____
_Field rules:_ The `adminConsoleServiceTemplate` field is optional and must be a Kubernetes https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.19/#service-v1-core[`Service` resource^].
____
...
====
This field is deprecated and will be removed in a later release.
The functionality is now redundant with Couchbase server 6.5.0 or greater.
====
...
====
This field is deprecated and will be removed in a later release.
The functionality is now provided by the <<spec-networking-adminconsoleservicetemplate>> property.
====
...
When the <<spec-networking-exposedfeatures>> property is set, the Operator will generate a service that exposes Couchbase services per-pod.
This field allows the definition of a base https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.19/#service-v1-core[`Service` resource].
The Operator will merge any configuration it controls on top of this template.
...
____
_Field rules:_ The `exposedFeatureServiceTemplate` field is optional and must be a Kubernetes https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.19/#service-v1-core[`Service` resource^].
____
...
====
This field is deprecated and will be removed in a later release.
The functionality is now provided by the <<spec-networking-exposedfeatureservicetemplate>> property.
====
...
==== 
This field is deprecated and will be removed in a later release.
The functionality is now provided by the <<spec-networking-adminconsoleservicetemplate>> and <<spec-networking-exposedfeatureservicetemplate>> properties.
====
...
This field defines any IPv4 prefixes that should be added to any load balancer services managed by the Operator.
This is a list of address ranges that are allowed to connect to the service.

____
_Field rules:_ The `loadBalancerSourceRanges` field is optional and must be a list of IPv4 address prefixes e.g. `10.0.0.0/8`.
____

=== `spec.networking.networkPlatform`

This field defines the underlying network platform in use.
Typically, this field does not need to be defined.

When `Istio`, this enables DNS host aliases in Couchbase server pods.
Without this support, Couchbase server may connect to local services over the pod IP address, not `localhost`, and will be dropped by Istio when running in mTLS mode.

____
_Field rules:_ The `networkPlatform` field is optional and must be `Istio`.
____
...
The DNS configuration can also be used with the `NodePort` external feature type.
...
____
_Field rules:_ The `domain` field is required when either the `exposedFeatureServiceType` or `adminConsoleServiceType` fields are configured as `LoadBalancer`.
____
...
This field specifies the number of nodes of this type that should be in the cluster. This allows the user to scale up different parts of the cluster as necessary. If this parameter is changed at run time, the Operator will xref:howto-couchbase-scale.adoc[automatically scale the cluster].
...
When set to `true` a `CouchbaseAutoscale` resource is created by the operator with the name of the enabled server configuration along with the name of the cluster.
For example, a server configuration named `default` will result in a `CouchbaseAutoscale` resource created named `default.cb-example`.
This resource conforms to Kubernetes `HorizontalPodAutoscaler` API and can be specified as a `scaleTargetRef` according to the https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.19/#horizontalpodautoscalerspec-v2beta2-autoscaling[HorizontalPodAutoscalerSpec] resource specification.

____
_Field rules:_ This field is optional and must be a boolean.  The default value is `false`.
____
...
The `env` field is defined by the Kubernetes https://v1-17.docs.kubernetes.io/docs/reference/generated/kubernetes-api/v1.17/#envvar-v1-core[`EnvVar`^] type.
...
Specified environment variables will be available in Couchbase server containers only.
The `envFrom` field is defined by the Kubernetes https://v1-17.docs.kubernetes.io/docs/reference/generated/kubernetes-api/v1.17/#envfromsource-v1-core[`EnvFromSource`^] type.
...
Pod resources are specified using a Kubernetes https://v1-17.docs.kubernetes.io/docs/reference/generated/kubernetes-api/v1.17/#resourcerequirements-v1-core[`ResourceRequirements`^] type:
...
The `pod` attribute is based on the Kubernetes https://v1-17.docs.kubernetes.io/docs/reference/generated/kubernetes-api/v1.17/#podtemplatespec-v1-core[`PodTemplateSpec`^] structure and allows all supported configuration parameters to be modified.
...
The `nameservers` sub-attribute is a list of DNS name server IP addresses.
...
If left unspecified, the dynamic admission controller will automatically populate it with the most recent container image that was available when the installed version of the Autonomous Operator was released.
...
(This is the default name given to the `ServiceAccount` when you run `cbopcfg` as part of the automated backup xref:howto-backup.adoc[setup procedure].
...
____
_Field rules:_ This field is optional and must be the name of a valid Kubernetes `Secret`.
____
...
This field defines affinity rules for the scheduling of xref:reference-couchbasebackup.adoc[`CouchbaseBackup`] and xref:reference-couchbasebackuprestore.adoc[`CouchbaseBackupRestore`] resources.

This field is formatted as per the https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector[Kubernetes node selectors^] documentation.
A node selector is a set of key/value pairs that a Kubernetes `Node` must be labeled with in order for backup and restore jobs to be scheduled onto.

____
_Field rules:_ This field is optional and must be a https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector[Kubernetes node selector^].
____

=== `spec.backup.tolerations`

This field defines toleration of anti-affinity for the scheduling of xref:reference-couchbasebackup.adoc[`CouchbaseBackup`] and xref:reference-couchbasebackuprestore.adoc[`CouchbaseBackupRestore`] resources.

This field is formatted as per the https://kubernetes.io/docs/concepts/configuration/taint-and-toleration/[Kubernetes taints and tolerations^] documentation.
Taints provide anti-affinity rules, forcing all pods to be unschedulable on nodes with that taint.
Tolerations allow backup and restore jobs to tolerate taints and be scheduled on these nodes that are now guaranteed to not have other pods running there.

____
_Field rules:_ This field is optional and must be a list of https://kubernetes.io/docs/concepts/configuration/taint-and-toleration/[Kubernetes tolerations^].
____
...
Volume claim templates are based on the https://v1-17.docs.kubernetes.io/docs/reference/generated/kubernetes-api/v1.17/#persistentvolumeclaim-v1-core[`PersistentVolumeClaim`^] Kubernetes type.
...
If set, this attribute defines a set of labels to be merged into persistent volume claims generated by this template.
The Operator reserves the right to overwrite any label as necessary for operation.

=== `spec.volumeClaimTemplates[].metadata.annotations`

If set, this attribute defines a set of annotations to be merged into persistent volume claims generated by this template.
The Operator reserves the right to overwrite any label as necessary for operation.
