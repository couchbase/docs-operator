...
In this discouraged scenario, there is no shared DNS between 2 Kubernetes clusters - you must use IP based addressing.
Kubernetes exposes Pods by using Kubernetes `NodePort` type services.
As there is no DNS, TLS is not supported, so you must maintain security between the 2 clusters using a VPN.
...
In some cases, Kubernetes removes the old port before it creates the new one, which causes a brief loss of connectivity.
