...
`couchbasecluster.spec.xdcr.remoteClusters.tls.secret` documents the secret format.
...
<.> **Inter-Kubernetes networking with forwarded DNS only:** the `couchbaseclusters.spec.servers.pod.spec.dnsPolicy` field tells Kubernetes to provide no default DNS configuration.

<.> **Inter-Kubernetes networking with forwarded DNS only:** the `couchbaseclusters.spec.servers.pod.spec.dnsConfig` field explicitly defines the local DNS name server to use.
...
`couchbaseclusters.spec.xdcr.remoteClusters.tls.secret`] documents the secret format.
