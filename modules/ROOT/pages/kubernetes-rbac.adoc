...
The Operator has the ability to automatically install the custom resource definition (CRD) (defining the `CouchbaseCluster` resource type).
This is a cluster-level operation.
For more information, refer to the xref:operator-config.adoc[Operator configuration documentation].
+
_Required Permissions_: `get`, `create`
...
+
_Required Permissions_: `create`, `patch`
...
persistentvolumes::
The Operator needs to be able to monitor the status of a persistent volume before monitoring the status of a persistent volume claim.  The persisten volume is required to derive the availability zone of a pod when using xref:server-groups.adoc[server groups].
+
_Required Permissions_: `get`, `watch`
...
+
_Required Permissions_: `get`, `create`, `delete`
