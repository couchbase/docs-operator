...
```
The first policy update (`add-cluster-role-to-user`) allows you to be able to deploy the Operator. It turns out that the Operator needs some non-standard permissions to create CRD's and watch for events on CouchbaseCluster objects and this policy update allows you to do that. The second policy update (`add-scc-to-user`) allows our Couchbase containers to start Couchbase as root.
