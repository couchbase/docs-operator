...
This field allows us to specify the API Version for our integration. As our integration changes over time we can change the API Version whenever we add new features. For any given release we will specify the API Versions supported by that operator in our documentation. We will also always recommend that users upgrade to the latest API Version when possible.

*Field rules:* This field is required and must be set to valid API version. The value of this field cannot be changed after the cluster is created.

**kind**

Specifies that this Kubernetes configuration will use our custom Couchbase controller to manage the cluster.
...
Allows setting a name for the cluster and a namespace that the cluster should be deployed in.
...
Specifies the image that should be used.

*Field rules:* The ```baseImage``` is required and should be set to "couchbase/server" unless Kubernetes should be pulling the Couchbase Server docker container from a different location. The value of this field cannot be changed after the cluster is created.

**version**

The version number of Couchbase Server to install. This should match an available version number in the Couchbase docker repository. It may never be changed to a lower version number that the version that is currently running.

*Field rules:* The ```version``` is required and should be set to the version of Couchbase Server that should be used to build the cluster. At the time of this writing the Couchbase Operator has only been tested with enterprise-5.0.1. The value of this field cannot be changed after the cluster is created.
...
Anti-affinity allows the user to specify whether or not two Pods in this cluster can be deployed on the same Kubernetes Node. In a production setting this parameter should always be set to true in order to reduce the chance of data-loss if a Kubernetes Node crashes.
...
The authSecret specifies the name of a Kubernetes Secret that should be used as for the username and password of the Couchbase super-user.

*Field rules:* The ```authSecret``` is required and should reference the name of a [Kubernetes Secret](https://kubernetes.io/docs/concepts/configuration/secret/) that already exists. The value of this field cannot be changed after the cluster is created.
...
*Field rules:* The ```adminConsoleServices``` list is not required and defaults to the empty list if not specified. An empty list means any node in the cluster may be choosen when connecting to the Administration Console.
...
The amount of memory to assign the the data service if present on a specific Couchbase node in MiB.

*Field rules:* The ```dataServiceMemoryQuota``` field is required and must be set to be greater than or equal to 256. Users should also keep in mind that the sum of all memory quotas must be no more than 80% of a pods available memory.

**indexServiceMemoryQuota**

The amount of memory to assign the the index service if present on a specific Couchbase node in MiB.

*Field rules:* The ```indexServiceMemoryQuota``` field is required and must be set to be greater than or equal to 256. Users should also keep in mind that the sum of all memory quotas must be no more than 80% of a pods available memory.
...
*Field rules:* The ```searchServiceMemoryQuota``` field is required and must be set to be greater than or equal to 256. Users should also keep in mind that the sum of all memory quotas must be no more than 80% of a pods available memory.

**indexStorageType**

Specifies the backend storage type to use for the index service. If the cluster already contains a Couchbase Server instance running the index service then this parameter cannot be changed until all instances running the index service are removed.
...
Sets the IO priority of background threads for this bucket.
...
*Specification rules:* The ```servers``` portion of the specification is required and must always contain at least one server definition. The must also be at least one definition that contains the data service.
...
Gives a name for this group of servers.
